...
export default function Repas() {
  ...
  async function ajouterRepasLibre(e) {
    ...
    await supabase.from('repas').insert([{ user_id: userId, nom, type, date_repas: aujourdHui() }])
    setNom('')
    charger()
  }
    ...
    <div>
      ...
      {(optionsDuType.length === 0 || modeLibre) && (
        <form onSubmit={ajouterRepasLibre} className="card flex flex-col gap-3 mb-6">
          <input value={nom} onChange={(e) => setNom(e.target.value)} placeholder="Ex: Poulet riz brocolis" className="border rounded-lg px-3 py-2" />
          <div className="flex gap-2">
            <button type="submit" className="btn-primary flex-1">Ajouter</button>
            {optionsDuType.length > 0 && <button type="button" onClick={() => setModeLibre(false)} className="px-4 py-2 rounded-xl bg-gray-100 text-sm font-medium">Annuler</button>}
          </div>
        </form>
      )}
      ...
      {loading ? <p className="text-gray-400">Chargement...</p> : (
        <div className="flex flex-col gap-3">
          ...
          {repas.length === 0 && <p className="text-gray-400 text-center py-8">Aucun repas ajouté pour aujourd'hui.</p>}
        </div>
      )}
    </div>
  )
