...
export default function Repas() {
  ...
  async function onResultatReconnaissance(aliments) {
    setShowReconnaissancePhoto(false)
    if (!userId || !aliments?.length) return
    for (const a of aliments) {
      await supabase.from('repas').insert([{
        ...
        date_repas: dateSelectionnee,
        kcal_libre: a.kcal,
        proteines_libre: a.proteines_g,
        glucides_libre: a.glucides_g,
        lipides_libre: a.lipides_g,
        quantite_g: a.quantite_g,
      }])
    }
    toast(`${aliments.length} aliment${aliments.length > 1 ? 's' : ''} ajouté${aliments.length > 1 ? 's' : ''} ✓`)
    charger()
  }
  ...
  return (
    <div>
      ...
      {showReconnaissancePhoto && <ReconnaissancePhoto onResultat={onResultatReconnaissance} onFermer={() => setShowReconnaissancePhoto(false)} />}
        ...
        <>
          ...
          <div style={{ borderTop: '1px solid var(--border)', paddingTop: '16px' }}>
            ...
            <FormulaireSaisieLibre
              ...
              onReconnaissancePhoto={() => setShowReconnaissancePhoto(true)}
              onAnnuler={null}
            />
          </div>
        </>
...
function FormulaireSaisieLibre({ nom, setNom, kcalLibre, setKcalLibre, proteinesLibre, setProteinesLibre,
  ...
  onSubmit, onScanner, onReconnaissancePhoto, onAnnuler }) {
  ...
  return (
    <form onSubmit={onSubmit} className="card flex flex-col gap-3 mb-6">
      <div className="flex gap-2">
        ...
        <button type="button" onClick={onReconnaissancePhoto}
          className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium border"
