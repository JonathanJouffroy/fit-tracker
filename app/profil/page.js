...
export default function Profil() {
  ...
  async function chargerTout() {
    ...
    const totalConso = (repasJour || []).reduce((acc, r) => acc + (r.options_repas?.kcal || 0), 0)
    ...
    if (poidsCorps) {
      const { data: logsJour } = await supabase.from('seances_log')
        .select('*, exercices(repetitions, poids_charge_kg)')
        .eq('user_id', user.id).eq('date_seance', today)
      const parExo = {}
      logsJour?.forEach((log) => {
        if (!log.exercices) return
        if (!parExo[log.exercice_id]) parExo[log.exercice_id] = { count: 0, exo: log.exercices }
        parExo[log.exercice_id].count += 1
      })
      totalBrule = Object.values(parExo).reduce((acc, { count, exo }) =>
        acc + calculerCaloriesExercice({ series: count, repetitions: exo.repetitions, poidsCharge: exo.poids_charge_kg, poidsCorps }), 0)
    }
    setCaloriesBrulees(totalBrule)
    ...
    const { data: repas7 } = await supabase.from('repas')
      .select('date_repas, options_repas(kcal)').eq('user_id', user.id)
      .gte('date_repas', dates7[0]).lte('date_repas', dates7[6])

    const { data: logs7 } = await supabase.from('seances_log')
      .select('date_seance, exercices(repetitions, poids_charge_kg)')
      .eq('user_id', user.id)
      .gte('date_seance', dates7[0]).lte('date_seance', dates7[6])

    const stat7 = dates7.map((date) => {
      const consomme = (repas7 || []).filter(r => r.date_repas === date).reduce((a, r) => a + (r.options_repas?.kcal || 0), 0)
      let brule = 0
      if (poidsCorps) {
        const parExo = {}
        logs7?.filter(l => l.date_seance === date).forEach((l) => {
          if (!l.exercices) return
          if (!parExo[l.exercice_id]) parExo[l.exercice_id] = { count: 0, exo: l.exercices }
          parExo[l.exercice_id].count += 1
        })
        brule = Object.values(parExo).reduce((a, { count, exo }) =>
          a + calculerCaloriesExercice({ series: count, repetitions: exo.repetitions, poidsCharge: exo.poids_charge_kg, poidsCorps }), 0)
      }
      return { date, consomme, brule }
    })
    ...
    const { data: repasMois } = await supabase.from('repas')
      .select('date_repas, options_repas(kcal)').eq('user_id', user.id).gte('date_repas', debutMois)
    const { data: logsMois } = await supabase.from('seances_log')
      .select('date_seance, exercices(repetitions, poids_charge_kg)')
    ...
    const statMois = joursUniques.map((date) => {
      const consomme = (repasMois || []).filter(r => r.date_repas === date).reduce((a, r) => a + (r.options_repas?.kcal || 0), 0)
      let brule = 0
      if (poidsCorps) {
        const parExo = {}
        logsMois?.filter(l => l.date_seance === date).forEach((l) => {
          if (!l.exercices) return
          if (!parExo[l.exercice_id]) parExo[l.exercice_id] = { count: 0, exo: l.exercices }
          parExo[l.exercice_id].count += 1
        })
