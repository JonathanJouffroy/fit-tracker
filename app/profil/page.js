...
export default function Profil() {
  ...
  // Calories brûlées aujourd'hui (séries d'exercices loguées)
  ...
  async function chargerTout() {
    ...
    // Calories brûlées aujourd'hui : on regroupe les séries loguées par exercice,
    // puis on applique la même formule MET que sur la page séance
    const poidsCorpsActuel = mesures?.[0]?.poids_kg
    if (poidsCorpsActuel) {
      const { data: logsJour } = await supabase
        .from('seances_log')
      ...
      logsJour?.forEach((log) => {
        ...
        if (!parExercice[log.exercice_id]) parExercice[log.exercice_id] = { count: 0, exo: log.exercices }
        parExercice[log.exercice_id].count += 1
      })

      const totalBrule = Object.values(parExercice).reduce((acc, { count, exo }) => {
        return acc + calculerCaloriesExercice({
          series: count,
          repetitions: exo.repetitions,
          poidsCharge: exo.poids_charge_kg,
          poidsCorps: poidsCorpsActuel,
        })
      }, 0)
        ...
        <>
          ...
          {resultatCalories && !editionProfil && (
            <div className="mb-6">
              ...
              />
              <div className="flex gap-2 mt-3 text-xs text-gray-500 justify-center flex-wrap">
                ...
                {caloriesBrulees > 0 && (
                  <>
                    <span>·</span>
                    <span className="text-green-600 font-medium">+{caloriesBrulees} kcal brûlées au sport</span>
                  </>
                )}
              </div>
            </div>
