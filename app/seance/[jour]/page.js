...
export default function SeanceJour() {
  ...
  const [nomsExistants, setNomsExistants] = useState([]) // pour l'autocomplétion
  ...
  async function chargerTout() {
    ...
    const [{ data: jourData }, { data: exosData }, { data: mesures }, { data: tousExos }] = await Promise.all([
      ...
      supabase.from('exercices').select('nom').eq('user_id', user.id), // pour autocomplétion
    ])
    ...
    // Noms uniques pour l'autocomplétion
    const noms = [...new Set((tousExos || []).map((e) => e.nom))].sort()
    setNomsExistants(noms)
  ...
  function kcalExercice(exo, nbSeries) {
    return calculerCaloriesExercice({ series: nbSeries, repetitions: exo.repetitions, poidsCharge: exo.poids_charge_kg, poidsCorps })
  }

  const kcalPrevu = exercices.reduce((a, e) => a + kcalExercice(e, e.series), 0)
  const kcalFait = exercices.reduce((a, e) => a + kcalExercice(e, seriesFaites[e.id] || 0), 0)
        ...
        {exercices.map((exo) => {
          ...
          return (
            <div key={exo.id} className="flex flex-col gap-3">
              <div className="card" style={{ opacity: termine ? 0.6 : 1 }}>
                  ...
                  <div className="flex-1">
                    ...
                    {poidsCorps && (
                      <p className="text-xs font-medium mt-0.5" style={{ color: 'var(--orange)' }}>
                        ~{kcalExercice(exo, exo.series)} kcal
                      </p>
                    )}
                  </div>
                  <button onClick={() => supprimerExercice(exo.id, exo.nom)}
                    className="text-sm ml-2" style={{ color: 'var(--text-faint)' }}>✕</button>
                </div>
                ...
                <div className="flex gap-1 mt-3">
                  {Array.from({ length: exo.series }).map((_, i) => (
                    <div key={i} className={`flex-1 h-1.5 rounded-full ${i < fait ? 'bg-green-500' : ''}`}
                      style={{ background: i < fait ? undefined : 'var(--surface-2)' }} />
                  ))}
                </div>
              </div>
