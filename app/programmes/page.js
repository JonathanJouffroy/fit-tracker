...
export default function Programmes() {
  ...
  async function appliquerProgramme(prog) {
    if (!confirm(`Appliquer "${prog.nom}" ? Les exercices actuels seront retirés de la semaine (ton historique de progression sera conservé).`)) return
    setApplicationEnCours(prog.id)
    // Délier les exercices existants de leur jour (ne pas supprimer pour conserver les logs)
    await supabase.from('exercices').update({ jour_id: null }).eq('user_id', userId)
