...
export default function Programmes() {
  ...
  // Exercices par jour : { jourId: [{nom, series, repetitions, repos_secondes, poids_charge_kg}] }
  const [exoParJour, setExoParJour] = useState({})
  const [jourActif, setJourActif] = useState(1) // jour sélectionné dans le formulaire
  ...
  async function charger() {
    ...
    // Noms d'exercices existants pour l'autocomplétion
  ...
  function ajouterExoJour(jourId) {
    setExoParJour((prev) => ({
      ...prev,
      [jourId]: [...(prev[jourId] || []), newExo()],
    }))
  }

  function supprimerExoJour(jourId, index) {
    setExoParJour((prev) => {
  ...
  function modifierExoJour(jourId, index, champ, valeur) {
    setExoParJour((prev) => {
      const liste = [...(prev[jourId] || [])]
  ...
  async function sauvegarderProgramme() {
    ...
    const totalExos = Object.values(exoParJour).flat().length
    ...
    if (error) { toast('Erreur lors de la création', 'error'); setSauvegarde(false); return }
    ...
    if (lignes.length > 0) {
      await supabase.from('programme_exercices').insert(lignes)
    }

    toast(`Programme "${nomProg}" créé ✓`)
  ...
  const nbExosJourActif = (exoParJour[jourActif] || []).length
      ...
      {modeCreation ? (
        <div className="flex flex-col gap-4">
          <div className="card flex flex-col gap-3">
            <p className="font-semibold" style={{ color: 'var(--text)' }}>Nouveau programme</p>
            <div>
          ...
          <div className="flex gap-1 overflow-x-auto pb-1">
            {JOURS.map((j) => {
              const nb = (exoParJour[j.id] || []).length
              const actif = jourActif === j.id
              return (
                <button key={j.id} onClick={() => setJourActif(j.id)}
                  className="flex-shrink-0 px-3 py-2 rounded-xl text-sm font-medium relative"
                  style={{
          ...
          <div className="card flex flex-col gap-3">
              ...
              <div key={i} className="flex flex-col gap-2 pb-3 border-b last:border-0 last:pb-0"
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1">
                    ...
                    <AutocompleteInput
                      value={exo.nom}
                      onChange={(v) => modifierExoJour(jourActif, i, 'nom', v)}
                      suggestions={nomsExistants}
                      placeholder="Ex: Squat, Développé couché..."
                    />
                  </div>
                  <button onClick={() => supprimerExoJour(jourActif, i)}
                ...
                <div className="flex gap-2">
                  <div className="flex-1">
                    ...
                    <input type="number" min="1" value={exo.series}
                      onChange={(e) => modifierExoJour(jourActif, i, 'series', e.target.value)}
                      className="input" />
                  </div>
                  <div className="flex-1">
                    <label className="label">Reps</label>
                    <input type="number" min="1" value={exo.repetitions}
                      onChange={(e) => modifierExoJour(jourActif, i, 'repetitions', e.target.value)}
                      className="input" />
                  </div>
                  <div className="flex-1">
                    <label className="label">Repos (s)</label>
                    <input type="number" min="0" step="15" value={exo.repos_secondes}
                      onChange={(e) => modifierExoJour(jourActif, i, 'repos_secondes', e.target.value)}
                      className="input" />
                  </div>
                  <div className="flex-1">
                    <label className="label">Poids (kg)</label>
                    <input type="number" min="0" step="0.5" value={exo.poids_charge_kg || ''}
                      onChange={(e) => modifierExoJour(jourActif, i, 'poids_charge_kg', e.target.value)}
            ...
            <button onClick={() => ajouterExoJour(jourActif)}
          ...
          {Object.keys(exoParJour).length > 0 && (
            <div className="card py-3">
              ...
              <div className="flex flex-col gap-1">
                {JOURS.filter(j => (exoParJour[j.id] || []).length > 0).map(j => (
                  <div key={j.id} className="flex justify-between text-sm">
                    <span style={{ color: 'var(--text)' }}>{j.nom}</span>
                    <span style={{ color: 'var(--text-muted)' }}>
                      {(exoParJour[j.id] || []).filter(e => e.nom.trim()).length} exercice{(exoParJour[j.id] || []).filter(e => e.nom.trim()).length > 1 ? 's' : ''}
                    </span>
                  </div>
                ))}
              </div>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={sauvegarderProgramme} disabled={sauvegarde || !nomProg.trim()}
              className="flex-[2] btn-primary text-sm py-3 disabled:opacity-50">
              {sauvegarde ? 'Création...' : '✓ Créer le programme'}
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
          <button onClick={ouvrirCreation} className="btn-primary w-full">
            + Créer un nouveau programme
          </button>

          {/* Sauvegarder semaine actuelle */}
...
// -------- Sous-composant : sauvegarder la semaine actuelle --------
...
// -------- Sous-composant : carte programme --------
