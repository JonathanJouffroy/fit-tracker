...
// Dessine les zones actives avec un effet chaleur corporelle (dégradé thermique rouge→orange→jaune)
function dessinerZones(ctx, zones, zonesActives, x, y, w, h) {
  Object.entries(zones).forEach(([nom, ellipses]) => {
    ...
    ellipses.forEach(({ cx, cy, rx, ry }) => {
      const px = x + cx * w
      const py = y + cy * h
      // Zone élargie pour le dégradé doux
      const prx = rx * w * 1.8
      const pry = ry * h * 1.8

      ctx.save()

      // Dégradé externe (halo chaud, orange→transparent)
      const gradHalo = ctx.createRadialGradient(px, py, 0, px, py, Math.max(prx, pry))
      gradHalo.addColorStop(0,    'rgba(255, 60,  0, 0.85)')  // rouge vif au centre
      gradHalo.addColorStop(0.25, 'rgba(255, 80,  0, 0.75)')  // rouge-orange
      gradHalo.addColorStop(0.5,  'rgba(255, 140, 0, 0.55)')  // orange chaud
      gradHalo.addColorStop(0.75, 'rgba(200, 100, 0, 0.25)')  // orange foncé
      gradHalo.addColorStop(1.0,  'rgba(150,  60, 0, 0)')     // transparent

      // Utiliser l'ellipse via scale
      ctx.translate(px, py)
      ctx.scale(prx / Math.max(prx, pry), pry / Math.max(prx, pry))
      ctx.beginPath()
      ctx.arc(0, 0, Math.max(prx, pry), 0, Math.PI * 2)
