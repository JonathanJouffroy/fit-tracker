...
export function analyserProgression(sessions) {
  ...
  // Filtrer les séances avec au moins une série loguée (avec ou sans poids)
  const avecDonnees = sessions.filter(s =>
    s.series?.length > 0 || s.reps_max > 0
  )
  ...
  if (!estAssistance) {
    ...
    // SAUF si les reps sont stables sur 2+ séances → augmenter la charge
    if (repsDerniere >= fourchette.min && repsDerniere < fourchette.max) {
      const repsStables = seances.length >= 2 &&
        seances.slice(-2).every(s => Math.round(s.repsMoyennes) >= repsDerniere - 1)

      if (repsStables && repsDerniere >= fourchette.min + 2) {
        // Reps stables depuis 2 séances → augmenter la charge
        const nouveauPoids = arrondir(derniere.poidsMax + 2.5)
        return {
          statut: 'augmenter_charge',
          message: `Tu es stable à ${repsDerniere} reps sur ${derniere.poidsMax}kg. C'est le moment d'augmenter — passe à ${nouveauPoids}kg × ${fourchette.min} reps.`,
          couleur: '#22c55e',
