...
export function analyserProgression(sessions) {
  ...
  const seances = avecDonnees.map(s => {
    if (s.series && s.series.length > 0) {
      ...
      if (seriesAvecPoids.length === 0) return { date: s.date, poidsMax: 0, poidsMoyen: 0, repsMoyennes: 8, decrochage: false, consistance: 1, nbSeries: s.series.length, estAssistance: false }

      const estAssistance = seriesAvecPoids.every(sr => sr.poids < 0)
      // Pour l'assistance : on travaille avec les valeurs absolues, mais la logique est inversée
      const poidsAbs = seriesAvecPoids.map(sr => Math.abs(sr.poids))
      const poidsMax = estAssistance
        ? -Math.min(...poidsAbs) // "max" en assistance = moins d'aide = valeur la moins négative
      ...
      // Décrochage : pour la charge, la dernière série est plus faible que la première
      // Pour l'assistance, la dernière série a plus d'assistance (plus négative) que la première
      const premier = seriesAvecPoids[0].poids
      const dernier = seriesAvecPoids[seriesAvecPoids.length - 1].poids
      const decrochage = seriesAvecPoids.length > 1 && (
        estAssistance ? dernier < premier : dernier < premier
      )
      ...
      return { date: s.date, poidsMax, poidsMoyen, repsMoyennes, decrochage, consistance, nbSeries: s.series.length, estAssistance }
    } else {
      const estAssistance = s.poids_max < 0
      return { date: s.date, poidsMax: s.poids_max, poidsMoyen: s.poids_max, repsMoyennes: s.reps_max || 8, decrochage: false, consistance: 1, nbSeries: 1, estAssistance }
    }
  })
  ...
  if (joursDepuisDerniere > 14) {
    ...
    return {
      ...
      message: estAssistance
        ? `Aucune séance depuis ${joursDepuisDerniere} jours. Reprends avec ${poidsReprise}kg d'assistance pour éviter les blessures.`
      ...
      couleur: '#f59e0b',
      suggestion: { poids: estAssistance ? -poidsReprise : poidsReprise, raison: 'Reprise après pause' }
    }
  }

  // Décrochage en fin de séance
  if (derniere.decrochage && derniere.consistance < 0.92) {
    ...
    return {
      ...
      message: estAssistance
        ...
        : `Tu as dû baisser le poids en fin de séance (consistance ${Math.round(derniere.consistance * 100)}%). Consolide à ${suggestion}kg sur toutes les séries avant d'augmenter.`,
      couleur: '#f59e0b',
      suggestion: { poids: estAssistance ? -suggestion : suggestion, raison: 'Consolidation des séries' }
    }
  }
  ...
  if (regression) {
    const suggestion = estAssistance
      ? arrondir(Math.abs(derniere.poidsMax) * 1.05)
      : arrondir(derniere.poidsMax * 0.95)
    return {
      statut: 'regression',
      message: estAssistance
        ? `Tu as eu besoin de plus d'assistance sur les dernières séances. Repose-toi et reprends avec ${suggestion}kg d'assistance.`
        : `Ton poids a baissé sur les dernières séances. Vérifie ta récupération. Reste à ${suggestion}kg pour consolider.`,
      couleur: '#ef4444',
      suggestion: { poids: estAssistance ? -suggestion : suggestion, raison: 'Consolidation avant nouvelle progression' }
    }
  }
  ...
  if (plateau) {
    if (derniere.consistance >= 0.95) {
      const suggestion = estAssistance
        ? arrondir(Math.max(0, Math.abs(derniere.poidsMax) - 5))
        : arrondir(derniere.poidsMax + 2.5)
      return {
        statut: 'plateau',
        message: estAssistance
          ? `Tu es stable à ${Math.abs(derniere.poidsMax)}kg d'assistance avec une bonne régularité. Essaie de réduire à ${suggestion}kg d'assistance !`
          : `Tu es stable à ${derniere.poidsMax}kg avec une bonne consistance. C'est le moment de passer à ${suggestion}kg !`,
        couleur: '#f59e0b',
        suggestion: { poids: estAssistance ? -suggestion : suggestion, raison: 'Dépassement du plateau' }
      }
    } else {
      return {
        statut: 'plateau',
        message: estAssistance
          ? `Tu es à ${Math.abs(derniere.poidsMax)}kg d'assistance mais avec de la variation entre tes séries. Stabilise d'abord.`
          : `Tu es à ${derniere.poidsMax}kg mais avec une variation entre tes séries. Stabilise avant d'augmenter.`,
        couleur: '#f59e0b',
        suggestion: { poids: derniere.poidsMax, raison: 'Améliorer la consistance' }
      }
    }
  }

  if (progressionPropre) {
    const increment = Math.abs(delta) <= 2.5 ? Math.abs(delta) : 2.5
    const suggestion = estAssistance
      ? arrondir(Math.max(0, Math.abs(derniere.poidsMax) - increment))
      : arrondir(derniere.poidsMax + increment)
    return {
      statut: 'progression',
      message: estAssistance
        ? `Belle progression ! Tu réduis l'assistance régulièrement. Essaie ${suggestion}kg d'assistance.`
        : `Belle progression ! +${delta}kg depuis la dernière séance. Continue avec ${suggestion}kg.`,
      couleur: '#22c55e',
      suggestion: { poids: estAssistance ? -suggestion : suggestion, raison: 'Continuation de la progression' }
    }
  }
  ...
  if (delta > 0 && derniere.consistance < 0.92) {
    return {
      ...
      message: estAssistance
        ? `Tu progresses mais avec de la variation entre tes séries. Consolide avant de réduire encore l'assistance.`
        : `Tu as progressé en poids max mais avec de la variation entre tes séries. Consolide avant d'augmenter.`,
      couleur: '#f59e0b',
  ...
  const suggestion = estAssistance
    ? arrondir(Math.max(0, Math.abs(derniere.poidsMax) - 2.5))
    : arrondir(derniere.poidsMax + 2.5)
  return {
    statut: 'stable',
    message: estAssistance
      ? `Tu te maintiens à ${Math.abs(derniere.poidsMax)}kg d'assistance. Essaie de réduire à ${suggestion}kg !`
      : `Tu te maintiens autour de ${derniere.poidsMax}kg. Essaie ${suggestion}kg sur ta prochaine séance.`,
    couleur: '#3B82F6',
    suggestion: { poids: estAssistance ? -suggestion : suggestion, raison: 'Prochaine étape' }
  }
}
