-- ============================================
-- FIT TRACKER — SCRIPT COMPLET (TOUT-EN-UN)
-- A lancer en une seule fois dans Supabase > SQL Editor
-- Remplace schema.sql + options_repas.sql + options_repas_suite.sql
-- ============================================

-- Reset propre : supprime tout si ça existe déjà (évite les erreurs "déjà existant")
...
-- Repas du jour (liste simple), lié à l'option choisie le cas échéant
create table repas (
  id bigint generated always as identity primary key,
  nom text not null,
...
-- ============================================
-- SÉCURITÉ (RLS ouvert pour démarrer simple)
-- ============================================
...
with l1 as (
  insert into options_repas (type, nom, profil, poids_total_g, kcal, proteines_g, glucides_g, lipides_g, ordre)
  values ('dejeuner', 'L''Assiette Poulet, Riz & Haricots verts', 'Un grand classique de la nutrition, riche en fibres digestes et micro-nutriments, très faible en lipides', 550, 505, 38, 64, 8, 1)
  returning id
)
insert into options_repas_ingredients (option_repas_id, nom, quantite, kcal, proteines_g, glucides_g, lipides_g, ordre)
select id, 'Filet de poulet grillé', '150g', 165, 33, 0, 3.5, 1 from l1
union all select id, 'Riz basmati (cuit)', '200g', 260, 5, 56, 0.5, 2 from l1
union all select id, 'Haricots verts (vapeur ou poêlés)', '200g', 45, 0, 8, 0, 3 from l1
union all select id, 'Huile d''olive (cuisson)', '4g (1 c. à café rase)', 35, 0, 0, 4, 4 from l1;

with l2 as (
  insert into options_repas (type, nom, profil, poids_total_g, kcal, proteines_g, glucides_g, lipides_g, ordre)
  values ('dejeuner', 'Le Rice Bowl Frais au Saumon', 'Une salade de riz froide façon "Poke", riche en Oméga-3 et très fraîche', 470, 585, 31, 62, 22, 2)
  returning id
)
insert into options_repas_ingredients (option_repas_id, nom, quantite, kcal, proteines_g, glucides_g, lipides_g, ordre)
select id, 'Pavé de saumon frais (cuit et refroidi)', '120g', 250, 24, 0, 17, 1 from l2
union all select id, 'Riz basmati (cuit et refroidi)', '200g', 260, 5, 56, 0.5, 2 from l2
union all select id, 'Dés de tomates & concombres', '150g', 30, 2, 6, 0, 3 from l2
union all select id, 'Huile d''olive (vinaigrette)', '5g (1 c. à café)', 45, 0, 0, 5, 4 from l2;

with l3 as (
  insert into options_repas (type, nom, profil, poids_total_g, kcal, proteines_g, glucides_g, lipides_g, ordre)
  values ('dejeuner', 'Le Duo Steaks 5% MG & Patate douce', 'Idéal pour couper la faim avec les fibres de la patate douce et des haricots verts', 600, 520, 45, 48, 14, 3)
  returning id
)
insert into options_repas_ingredients (option_repas_id, nom, quantite, kcal, proteines_g, glucides_g, lipides_g, ordre)
select id, 'Steaks hachés 5% MG', '2 steaks (200g)', 260, 42, 0, 10, 1 from l3
union all select id, 'Écrasé de patate douce (cuite)', '200g', 170, 3, 40, 0, 2 from l3
union all select id, 'Haricots verts à l''ail', '200g', 45, 0, 8, 0, 3 from l3
union all select id, 'Huile d''olive (cuisson/assaisonnement)', '5g (1 c. à café)', 45, 0, 0, 5, 4 from l3;
...
with d1 as (
  insert into options_repas (type, nom, profil, poids_total_g, kcal, proteines_g, glucides_g, lipides_g, ordre)
  values ('diner', 'La Salade Complète Pâtes, Thon & Œufs', 'Frais, rapide (sans cuisson si les œufs sont prêts), parfait pour les soirs d''été', 550, 490, 39, 51, 14, 1)
  returning id
)
insert into options_repas_ingredients (option_repas_id, nom, quantite, kcal, proteines_g, glucides_g, lipides_g, ordre)
select id, 'Pâtes complètes (cuites et refroidies)', '150g', 190, 7, 38, 1, 1 from d1
union all select id, 'Thon au naturel (égoutté)', '1 boîte (~100g)', 110, 25, 0, 1, 2 from d1
union all select id, 'Œufs durs', '2 œufs entiers (~120g)', 150, 13, 1, 10, 3 from d1
union all select id, 'Tomates cerises & Cornichons', '150g', 25, 1, 4, 0, 4 from d1
union all select id, 'Jus de citron & Filet d''huile d''olive', '2g', 15, 0, 0, 2, 5 from d1;

with d2 as (
  insert into options_repas (type, nom, profil, poids_total_g, kcal, proteines_g, glucides_g, lipides_g, ordre)
  values ('diner', 'Le Mexican Bowl au Poulet', 'Très digeste, équilibré, apporte des fibres et de la couleur dans l''assiette', 580, 485, 39, 56, 11, 2)
  returning id
)
insert into options_repas_ingredients (option_repas_id, nom, quantite, kcal, proteines_g, glucides_g, lipides_g, ordre)
select id, 'Émincé de poulet (cuit)', '120g', 135, 26, 0, 3, 1 from d2
union all select id, 'Riz basmati (cuit)', '150g', 195, 4, 42, 0, 2 from d2
union all select id, 'Mélange Maïs & Haricots rouges', '100g', 95, 5, 14, 1, 3 from d2
union all select id, 'Salade verte & Tomates', '150g', 20, 1, 3, 0, 4 from d2
union all select id, 'Huile d''olive (assaisonnement)', '4g (1 c. à café rase)', 40, 0, 0, 4, 5 from d2;

with d3 as (
  insert into options_repas (type, nom, profil, poids_total_g, kcal, proteines_g, glucides_g, lipides_g, ordre)
  values ('diner', 'Pavé de Saumon et Poêlée de Légumes verts', 'Ultra-léger en glucides, idéal pour les jours sans entraînement ou pour un sommeil réparateur', 470, 415, 27, 24, 23, 3)
  returning id
)
insert into options_repas_ingredients (option_repas_id, nom, quantite, kcal, proteines_g, glucides_g, lipides_g, ordre)
select id, 'Pavé de saumon frais (au four ou à la poêle)', '120g', 250, 24, 0, 17, 1 from d3
union all select id, 'Riz basmati (cuit)', '50g (petite portion)', 65, 1, 14, 0, 2 from d3
union all select id, 'Haricots verts sautés à l''ail', '200g', 45, 2, 8, 0, 3 from d3
union all select id, 'Huile d''olive (cuisson)', '6g', 55, 0, 0, 6, 4 from d3;
...
with c1 as (
  ...
  values ('collation', 'Le Shaker Protéiné Express & Banane', 'Idéal post-entraînement ou pour un encas rapide. Simple, efficace et sans vaisselle', 430, 290, 27, 31, 3, 'Verser la whey et le lait d''amande directement dans un shaker manuel, secouer vigoureusement 15 secondes. Manger la banane entière à côté.', 1)
  returning id
)
insert into options_repas_ingredients (option_repas_id, nom, quantite, kcal, proteines_g, glucides_g, lipides_g, ordre)
select id, 'Whey protéine (1 dose standard)', '30g', 110, 24, 2, 1.5, 1 from c1
union all select id, 'Lait d''amande sans sucres ajoutés', '250g (ou ml)', 30, 1, 0, 1.5, 2 from c1
union all select id, 'Banane mûre', '1 moyenne (~150g)', 150, 2, 29, 0, 3 from c1;

with c2 as (
  insert into options_repas (type, nom, profil, poids_total_g, kcal, proteines_g, glucides_g, lipides_g, note_preparation, ordre)
  values ('collation', 'L''Encas Minute Skyr, Choco & Amandes', 'Satiété durable grâce aux protéines lentes et aux bons lipides. Super gourmand', 370, 325, 24, 25, 14, 'Couper le chocolat au couteau et l''ajouter avec les amandes dans le pot de Skyr. Manger la pomme à côté.', 2)
  returning id
)
insert into options_repas_ingredients (option_repas_id, nom, quantite, kcal, proteines_g, glucides_g, lipides_g, ordre)
select id, 'Skyr Nature', '200g', 114, 20, 7, 0, 1 from c2
union all select id, 'Chocolat noir 85%', '1 carré (~10g)', 55, 1, 2, 5, 2 from c2
union all select id, 'Amandes', '1 petite poignée (15g)', 90, 3, 1, 8, 3 from c2
union all select id, 'Pomme', '1 moyenne (~150g)', 66, 0, 15, 1, 4 from c2;
