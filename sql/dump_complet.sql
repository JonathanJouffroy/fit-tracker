...
create table repas (
  ...
  kcal_libre int default null, -- calories saisies manuellement (repas libre)
  created_at timestamp default now()
);
...
select
  ...
  sl.date_seance,
  count(*) as nb_series,
  max(sl.poids_kg) as poids_max,
  avg(sl.poids_kg) as poids_moyen,
  sum(sl.poids_kg * sl.repetitions_faites) as volume_total
from seances_log sl
join exercices e on e.id = sl.exercice_id
where sl.poids_kg is not null and sl.poids_kg > 0
group by sl.user_id, sl.exercice_id, e.nom, sl.date_seance
order by sl.date_seance desc;

-- ============================================
-- PROGRAMMES RÉUTILISABLES
-- ============================================

-- Programmes (fixes partagés + perso par user)
create table programmes (
  id bigint generated always as identity primary key,
  user_id uuid references auth.users(id) on delete cascade, -- null = programme fixe partagé
  nom text not null,
  description text,
...
-- Exercices modèles dans un programme (lié à un jour + un programme)
...
-- Programmes fixes : lecture publique
create policy "lecture programmes fixes" on programmes for select using (est_fixe = true);
-- Programmes perso : accès par user
create policy "user programmes" on programmes for all
  using (auth.uid() = user_id) with check (auth.uid() = user_id);
-- Exercices des programmes fixes : lecture publique
create policy "lecture programme_exercices fixes" on programme_exercices for select
  using (exists (select 1 from programmes p where p.id = programme_id and p.est_fixe = true));
-- Exercices des programmes perso : accès par user
create policy "user programme_exercices" on programme_exercices for all
  using (exists (select 1 from programmes p where p.id = programme_id and p.user_id = auth.uid()))
  with check (exists (select 1 from programmes p where p.id = programme_id and p.user_id = auth.uid()));

-- ============================================
-- DONNÉES : PROGRAMMES FIXES
-- ============================================

-- Programme PPL (Push Pull Legs) — 6 jours
with prog as (
  insert into programmes (nom, description, est_fixe)
  values ('Push Pull Legs (PPL)', 'Programme 6 jours : Poussée / Tirage / Jambes, répété 2 fois par semaine.', true)
  returning id
)
...
from prog, (values
  (1, 'Développé couché', 4, 8, 90, 1),
  (1, 'Développé militaire', 3, 10, 75, 2),
  (1, 'Élévations latérales', 3, 15, 45, 3),
  (1, 'Triceps poulie', 3, 12, 60, 4),
  (2, 'Tractions', 4, 8, 90, 1),
  (2, 'Rowing haltère', 3, 10, 75, 2),
  (2, 'Curl biceps', 3, 12, 60, 3),
  (2, 'Face pull', 3, 15, 45, 4),
  (3, 'Squat barre', 4, 8, 120, 1),
  (3, 'Presse à cuisses', 3, 10, 90, 2),
  (3, 'Fentes', 3, 12, 60, 3),
  (3, 'Mollets debout', 4, 15, 45, 4),
  (4, 'Développé incliné', 4, 8, 90, 1),
  (4, 'Dips', 3, 10, 75, 2),
  (4, 'Élévations frontales', 3, 12, 45, 3),
  (5, 'Soulevé de terre', 4, 6, 120, 1),
  (5, 'Tirage vertical', 3, 10, 75, 2),
  (5, 'Rowing câble', 3, 12, 60, 3),
  (6, 'Leg press', 4, 10, 90, 1),
  (6, 'Leg curl', 3, 12, 60, 2),
  (6, 'Hip thrust', 3, 12, 75, 3)
) as exo(jour_num, nom, series, reps, repos, ordre)
join jours j on j.numero = exo.jour_num;

-- Programme Full Body — 3 jours (Lun/Mer/Ven)
with prog as (
  insert into programmes (nom, description, est_fixe)
  values ('Full Body 3 jours', 'Travail du corps entier 3 fois par semaine. Idéal pour les débutants.', true)
  returning id
)
insert into programme_exercices (programme_id, jour_id, nom, series, repetitions, repos_secondes, ordre)
select prog.id, j.id, exo.nom, exo.series, exo.reps, exo.repos, exo.ordre
from prog, (values
  (1, 'Squat', 3, 10, 90, 1),
  (1, 'Développé couché', 3, 10, 90, 2),
  (1, 'Rowing haltère', 3, 10, 75, 3),
  (1, 'Gainage', 3, 30, 45, 4),
  (3, 'Soulevé de terre roumain', 3, 10, 90, 1),
  (3, 'Développé militaire', 3, 10, 75, 2),
  (3, 'Tractions assistées', 3, 10, 75, 3),
  (3, 'Crunchs', 3, 15, 45, 4),
  (5, 'Fentes marchées', 3, 12, 75, 1),
  (5, 'Dips', 3, 10, 75, 2),
  (5, 'Curl biceps', 3, 12, 60, 3),
  (5, 'Planche latérale', 3, 20, 45, 4)
) as exo(jour_num, nom, series, reps, repos, ordre)
join jours j on j.numero = exo.jour_num;
...
with prog as (
  insert into programmes (nom, description, est_fixe)
  values ('Upper Lower 4 jours', 'Haut du corps / Bas du corps, 4 séances par semaine.', true)
  returning id
)
insert into programme_exercices (programme_id, jour_id, nom, series, repetitions, repos_secondes, ordre)
select prog.id, j.id, exo.nom, exo.series, exo.reps, exo.repos, exo.ordre
from prog, (values
  (1, 'Développé couché', 4, 8, 90, 1),
  (1, 'Tirage vertical', 4, 8, 90, 2),
  (1, 'Développé militaire', 3, 10, 75, 3),
  (1, 'Rowing câble', 3, 10, 75, 4),
  (2, 'Squat barre', 4, 8, 120, 1),
  (2, 'Soulevé de terre roumain', 3, 10, 90, 2),
  (2, 'Leg press', 3, 12, 75, 3),
  (2, 'Mollets', 4, 15, 45, 4),
  (4, 'Développé incliné haltères', 4, 10, 90, 1),
  (4, 'Tractions', 4, 8, 90, 2),
  (4, 'Curl biceps', 3, 12, 60, 3),
  (4, 'Triceps barre', 3, 12, 60, 4),
  (5, 'Soulevé de terre', 4, 6, 120, 1),
  (5, 'Fentes', 3, 12, 75, 2),
  (5, 'Leg curl', 3, 12, 60, 3),
  (5, 'Hip thrust', 3, 12, 75, 4)
) as exo(jour_num, nom, series, reps, repos, ordre)
join jours j on j.numero = exo.jour_num;
